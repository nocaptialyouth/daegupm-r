(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("daegu-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
  ...
  const districtOrder = ["중구","동구","서구","남구","북구","수성구","달서구","달성군","군위군"];
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("daegu-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정으로 저장이 막혀도 검색과 상세보기는 계속 동작합니다.
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.className = "drawer" + (state.district === "" ? " active" : "");
    allChip.dataset.district = "";
    allChip.innerHTML = `대구 전체 <span class="cnt">${data.length}</span>`;
    row.appendChild(allChip);
    
    districtOrder.forEach((gugun) => {
      if (!counts[gugun]) return;
      const chip = document.createElement("button");
      chip.className = "drawer" + (state.district === gugun ? " active" : "");
      chip.dataset.district = gugun;
      chip.innerHTML = `${gugun} <span class="cnt">${counts[gugun]}</span>`;
      row.appendChild(chip);
    });
    
    row.querySelectorAll(".drawer").forEach((el) => {
      el.addEventListener("click", () => {
        row.querySelectorAll(".drawer").forEach((x) => x.classList.remove("active"));
  ...
  function resetAll() {
    ...
    document.querySelectorAll("#districtRow .drawer").forEach((el) => {
